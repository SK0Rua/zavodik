...
export function registerCampaignCommandRoutes(
  ...
  app.post('/internal/campaigns', internalAuth, async (context) => {
    ...
    const input: CreateCampaignInput = {
      ...
      // Stop-point ladder + discovery filter (Roman's workflow 2026-08-27). Both
      // are normalized through the shared campaignFlow logic so a missing field
      // falls back to "full auto, no extra filtering".
      autoStage: normalizeAutoStage(typeof body.autoStage === 'string' ? body.autoStage : ''),
      discoveryFilter: normalizeDiscoveryFilter(body.discoveryFilter),
    };

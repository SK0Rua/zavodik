...
async function mineCaptures(
...
): Promise<{ logos: ImageRef[]; photos: ImageRef[]; notes: string[] }> {
  const notes: string[] = [];
  ...
  const logos: ImageRef[] = ranked.map((c) => ({
    url: c.url,
    kind: 'logo' as const,
...
export async function collectAssetsHandler(payload: JobPayload): Promise<void> {
  ...
  const offered = (payload.imageUrls ?? []) as unknown as ImageRef[];
  ...
  // ── Refresh the brand palette now that the images exist ──────────────────
  //
  // `enrich` runs brand extraction before this job, so on a first pass it sees
  // the captured pages but not the downloaded LOGO — and the logo is the
  // highest-authority colour evidence a business has. Re-running here is what
  // turns a photo-derived palette into a logo-derived one.
  //
  // `skipVoice` because nothing this job did changes what the business wrote
  // about itself, and a second subscription call for an unchanged answer is
  // waste; `preserveVoice` so the rewrite keeps the classification enrichment
  // already paid for.
  //
  // Non-fatal: assets are collected either way. A demo with photographs and a
  // stale palette is far better than one with neither.
  if (saved > 0) {
    try {
      ...
      log.warn('brand refresh after assets failed', { businessId, err: String(err).slice(0, 200) });
    }
  }
}

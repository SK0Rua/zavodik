...
const ASSESS_CITY_PAYLOAD = {
  required: { assessmentId: 'integer' },
  optional: ID_FIELDS.optional,
  allowAdditional: true,
} as const satisfies JobPayloadSchema;
...
export const JOB_DEFINITIONS = [
  ...
  // A throwaway gosom probe: the worker records its own failure on the
  // city_assessments row and does not rethrow, so a retry would only re-run a
  // scrape Roman can trigger again himself. One attempt, no auto-requeue.
  definition({ name: 'assess-city', workerGroup: 'core', agentCapability: 'none', retry: { limit: 0, delaySeconds: 0 }, expireInSeconds: STANDARD_EXPIRY, schedulingClass: 'upstream', displayName: 'оцінка міста', manualRequeue: false, payloadSchema: ASSESS_CITY_PAYLOAD }),

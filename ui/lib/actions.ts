...
import {
  BUILD_POLICY_LABELS, isActiveJobStatus, isActiveProjectState, normalizeBuildPolicy,
} from './buildPolicy';
import {
...
export async function createCampaign(formData: FormData): Promise<ActionResult> {
  ...
  const response = await factoryFetch('/internal/campaigns', {
    ...
    body: {
      ...
      radiusKm,
      autoBuild,
      // Stop-point ladder + discovery filter (Roman's workflow 2026-08-27). The
      // factory normalizes and persists them; a missing field falls back to the
      // previous "full auto, no extra filtering" behaviour.
      autoStage,
      discoveryFilter,
    },
  });

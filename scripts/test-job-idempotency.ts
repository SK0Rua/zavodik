...
import { DEFAULT_AUTO_STAGE, DEFAULT_DISCOVERY_FILTER } from '../src/orchestrator/campaignFlow.js';
...
await withDisposableFactoryDatabase(async ({ pool, db: testDb, boss }) => {
  ...
  await check('campaign creation and discovery enqueue commit exactly once', async () => {
    ...
    const input = {
      ...
      autoStage: DEFAULT_AUTO_STAGE,
      discoveryFilter: DEFAULT_DISCOVERY_FILTER,
    };

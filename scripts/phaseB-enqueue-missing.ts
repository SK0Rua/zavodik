 ...
 * through stages 4-8. Safe to run repeatedly: pg-boss singleton keys collapse a
 * duplicate while one is still active, and enrichment itself is idempotent
 * (it clears the business's facts before rewriting them).
 */
...
for (const r of rows) {
  const jobId = await enqueue('enrich', { businessId: r.id, campaignId });
  console.log(`  ${r.id} (${r.status}) -> ${jobId ?? 'collapsed (already queued)'}`);
}
process.exit(0);

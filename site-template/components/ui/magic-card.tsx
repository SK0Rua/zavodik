...
export function MagicCard(props: MagicCardProps) {
  ...
  // Template change: upstream used next-themes. A static demo site has no theme
  // switcher, so we read the document/media state directly and drop the dep.
  const [isDarkTheme, setIsDarkTheme] = useState(true)

  useEffect(() => {
    const root = document.documentElement
    const mq = window.matchMedia("(prefers-color-scheme: dark)")
    const compute = () => root.classList.contains("dark") || mq.matches
    setIsDarkTheme(compute())
    mq.addEventListener("change", () => setIsDarkTheme(compute()))
    return () => mq.removeEventListener("change", () => setIsDarkTheme(compute()))
  }, [])
  ...
  // Template change: upstream called useMotionTemplate inline inside JSX, one of
  // them behind `mode === "gradient"`. That is a conditional Hook call (breaks
  // when `mode` changes and trips react-hooks lint). Both are hoisted here.

...
### Демо і прев'ю — щоб «Переглянути збірку» і клієнтські лінки відкривались

`DEMO_BASE_URL` — це єдина адреса в усьому чеклісті, яку **мусить відкрити
чужий браузер**: за нею живуть і приватні демо, які ти надсилаєш бізнесу
(`src/workers/deploy.ts`), і прев'ю недеплойнутої збірки за кнопкою
«Переглянути збірку». Її обслуговує демо-сервер фабрики на **порту 8788**
(`src/lib/serveDir.ts`), опублікований лише на `127.0.0.1` — тобто без
явного маршруту в reverse-proxy кожне демо-посилання віддає 404.

```bash
# .env, або /settings → Система
DEMO_BASE_URL=https://<домен>/demo
```

**Маршрут обовʼязково має знімати префікс** — демо-сервер віддає токени зі
свого кореня (`/<token>/`, `/preview-<id>/`), а не з `/demo/<token>/`.

Ще одна пастка, дорожча за першу: сторінки Next-експорту просять свої
чанки за КОРЕНЕВИМИ шляхами (`/_next/...`, `/assets/...`), а не всередині
`/demo`. Демо-сервер уміє віднести їх до потрібної збірки за заголовком
`Referer` — але лише якщо запит до нього дійшов. Якщо `/_next/` у проксі
жорстко веде на UI, HTML відкриється, а сторінка буде **повністю без
стилів і картинок**, і жодна перевірка за статус-кодом цього не помітить.

**Як налаштовано на цьому сервері (nginx/openresty, 2026-09-07)** —
`/opt/1panel/apps/openresty/openresty/conf/conf.d/zavodik.skor.agency.conf`:
...
# `/_next/` і статика йдуть на демо-сервер, лише коли запит прийшов
# зі сторінки демо; решта — на UI, як і раніше.
location ^~ /_next/ {
    set $factory_backend "127.0.0.1:3000";
    if ($http_referer ~* "^https?://[^/]+/demo/") {
        set $factory_backend "127.0.0.1:8788";
    }
    proxy_pass http://$factory_backend;
}
```

Перевірка, яка ловить обидві пастки одразу (друга команда має віддати
`200` і `text/css`, а не HTML сторінки-404):

```bash
curl -I https://<домен>/demo/preview-<id>/
curl -I -e "https://<домен>/demo/preview-<id>/" https://<домен>/_next/static/css/<hash>.css
```
...
**Крок «Пропущено: стан змінився» на сторінці системи.** Це не помилка і не
успіх: доставка (збірка, перевірка, публікація) прийшла, коли проєкт або бізнес
уже перейшли далі, були зупинені чи перебудовані. Раніше такі кроки писались як
«Готово» і виглядали як зелена збірка, що нічого не зробила. Причина — у деталях
кроку. Якщо збірки немає, а хочеться нову: «Побудувати заново» на картці.

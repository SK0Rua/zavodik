...
> **ЗМІНЕНО 2026-08-22 (SPEC §2.5): FlowKit видалено з фабрики.** Кожен міст до
> Google Flow потребує живого залогіненого Chrome поза датацентром, а Роман не
> тримає нічого на маку. Відео тепер: автоматичний ffmpeg Ken Burns з реального
> фото + ручний wow-кліп (бриф і завантаження на картці бізнесу, вкладка Демо;
> `planHeroMedia` підхоплює найновіший asset `hero_clip`). Усі розділи нижче про
> FlowKit/Chrome-міст/`FLOWKIT_*` — історичний контекст, не інструкція.

...
2. **Wow-шлях (human-in-the-loop)**: на картці бізнесу (вкладка Демо) фабрика
   показує **відео-бриф** — готовий англомовний промпт (i2v, 8с, обмеження
   «нічого не вигадувати») + стартовий кадр (реальне hero-фото). Роман генерує
   у будь-якому інструменті і завантажує mp4 тією ж карткою; файл реєструється
   як asset `hero_clip` (`ai_generated`, `private_demo_only`,
   generator=manual-upload), і наступна збірка/ітерація використовує його
   замість Ken Burns автоматично (planHeroMedia бере найновіший hero_clip).

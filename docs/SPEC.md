...
**FlowKit** (форк Bl0ck154/flowkit, гілка omni-flash): генерація AI-відео через Google Flow (Veo) **по підписці Google AI**, без pay-per-use API. Готові skills під Claude Code (fk-gen-videos, fk-gen-images, fk-concat, fk-pipeline).

Що потрібно для роботи FlowKit (перевірено по архітектурі репо):

1. Chrome з розширенням (Load unpacked з `extension/`): захоплює Flow bearer-токен, вирішує reCAPTCHA, проксіює API;
2. залогінена вкладка `labs.google/fx/tools/flow` під акаунтом з підпискою Google AI; Chrome має бути запущений постійно;
3. Python-агент (FastAPI + SQLite, WebSocket до розширення, REST на :8100) + ffmpeg - саме цей REST смикає video-worker фабрики.

Розгортання: старт - на маку Романа (фабрика ходить на :8100 через Tailscale/VPN; мак має бути ввімкнений під час генерацій); пізніше опційно headful Chrome у Xvfb на хостингу для 24/7.

Використання у фабриці:

- hero-кліпи для демосайтів: image-to-video з РЕАЛЬНИХ фото бізнесу (оживлення evidence-фото, не вигадана картинка) + ambience-луп за нішею;
- згенеровані кліпи зберігаються як assets з позначкою `ai_generated` і тим самим правилом `private_demo_only`;
- fallback без FlowKit: безкоштовний сток (Pexels/Coverr) + Ken Burns/паралакс реальних фото.

Застереження: міст до Flow неофіційний (може ламатись при змінах Google) і потребує живого Chrome із залогіненим акаунтом - на маку Романа або в VM на хостингу; вирішується у фазі C.
...
12. **Відео через FlowKit** (Google Flow/Veo по підписці Google AI, Chrome-міст): hero-кліпи з реальних фото бізнесу; сток як fallback. Без pay-per-use відео-API. Chrome-міст на старті живе на маку Романа. (Розділ 2.5.)
